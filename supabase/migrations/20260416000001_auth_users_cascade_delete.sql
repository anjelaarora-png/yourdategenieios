...
DECLARE
  r RECORD;
BEGIN
  -- ── profiles (skip if table doesn't exist in this project) ────────────────
  ...
  -- ── user_roles (skip if table doesn't exist in this project) ──────────────
  IF EXISTS (SELECT 1 FROM information_schema.tables
  ...
  -- ── couples ───────────────────────────────────────────────────────────────
  -- couples.user_id_1 → public.users.user_id → auth.users.id cascade is already
  -- set up in 20260317200000. No direct auth.users FK needed here.
