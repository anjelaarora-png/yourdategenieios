...
class DatePlanGeneratorService: ObservableObject {
    ...
    func generateDatePlan(from questionnaire: QuestionnaireData) async throws -> [DatePlan] {
        ...
        do {
            ...
            for plan in plans {
                ...
                verifiedPlans.append(verifiedPlan)
            }
            
            // Prepend Google-verified starting point when user provided startingAddress
            if !questionnaire.startingAddress.isEmpty && Config.isGooglePlacesConfigured {
                if let startResult = try? await GooglePlacesService.shared.geocodeAddress(questionnaire.startingAddress) {
                    let startStop = DatePlanStop(
                        order: 1,
                        name: "Your location",
                        ...
                        emoji: "📍",
                        validated: true,
                        address: startResult.formattedAddress,
                        latitude: startResult.latitude,
                        longitude: startResult.longitude,
                        estimatedCostPerPerson: ""
                    )
                    verifiedPlans = verifiedPlans.map { plan in
                        let renumberedStops = plan.stops.enumerated().map { index, stop in
                            DatePlanStop(
                                order: index + 2,
                                name: stop.name,
                        ...
                        return DatePlan(
                            ...
                            totalDuration: plan.totalDuration,
                            estimatedCost: plan.estimatedCost,
                            stops: [startStop] + renumberedStops,
                            genieSecretTouch: plan.genieSecretTouch,
                            packingList: plan.packingList,
                            weatherNote: plan.weatherNote,
                            giftSuggestions: plan.giftSuggestions,
            ...
            let result = verifiedPlans
            await MainActor.run {
                self.generatedPlans = result
    ...
    /// Advances generationProgress from current value toward 0.85 over ~20 seconds so the loading UI never appears stuck.
    private func startProgressSimulation() {
        stopProgressSimulation()
        let targetCap: Double = 0.85
        let increment = (targetCap - 0.2) / 40
