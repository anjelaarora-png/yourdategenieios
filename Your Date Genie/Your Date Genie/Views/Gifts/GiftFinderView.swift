...
struct GiftFinderView: View {
    ...
    private func dateContextBadge(plan: DatePlan) -> some View {
        VStack(spacing: 8) {
            ...
            if !effectiveLocation.isEmpty {
                Button {
                    openGoogleMaps(query: "gift shop", near: effectiveLocation)
                } label: {
                    HStack(spacing: 6) {
    ...
    private func openGoogleMaps(query: String, near location: String) {
        let searchQuery = "\(query) near \(location)".addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? query
        
        // Try Google Maps app first, fallback to web
        if let googleMapsUrl = URL(string: "comgooglemaps://?q=\(searchQuery)"),
           UIApplication.shared.canOpenURL(googleMapsUrl) {
            UIApplication.shared.open(googleMapsUrl)
        } else if let webUrl = URL(string: "https://www.google.com/maps/search/\(searchQuery)") {
...
struct NearbyStoreCard: View {
    ...
    var body: some View {
        Button {
            openGoogleMaps(query: store.searchQuery, near: location)
        } label: {
            VStack(alignment: .leading, spacing: 10) {
                HStack(spacing: 10) {
                    Text(store.emoji)
