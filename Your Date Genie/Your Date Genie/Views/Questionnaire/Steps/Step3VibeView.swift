    ...
    var body: some View {
        ScrollView(showsIndicators: false) {
            VStack(alignment: .leading, spacing: 24) {
                ...
                // Time of Day
                VStack(alignment: .leading, spacing: 12) {
                    SectionHeader(emoji: "🕐", title: "What time of day?")
                    
                    LazyVGrid(columns: [
                        GridItem(.flexible()),
                        GridItem(.flexible())
                    ], spacing: 12) {
                        ForEach(QuestionnaireOptions.timeOfDay) { time in
                            OptionCardView(
                                item: time,
                                isSelected: data.timeOfDay == time.value,
                                onTap: { data.timeOfDay = time.value }
                            )
                        }
                    }
                }
